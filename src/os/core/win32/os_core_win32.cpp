...
internal OS_Key os_key_from_vk(u32 vk) {
    ...
    if (first_call) {
        ...
        keycode_table[VK_SHIFT]   = OS_KEY_SHIFT;
        keycode_table[VK_CONTROL] = OS_KEY_CONTROL;
        keycode_table[VK_MENU]    = OS_KEY_ALT;
        for (int i = 0; i < 12; i++) {
...
internal u64 os_file_last_write_time(OS_Handle handle) {
    u64 result = 0;
    FILETIME write_time;
    if (GetFileTime((HANDLE)handle, NULL, NULL, &write_time)) {
        result = ((u64)write_time.dwHighDateTime << 32) | write_time.dwLowDateTime;
    }
    return result;
}
